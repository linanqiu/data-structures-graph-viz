...
public class Graph extends JPanel {
  ...
  public Graph(ArrayList<String> cityNames, ArrayList<Integer> xy, ArrayList<String> pairs,
    ...
    for (int i = 0; i < names.size(); i++) {
      Vertex city = new Vertex(names.get(i), xy.get(i * 2), xy.get(i * 2 + 1));
  ...
  public void paint(Graphics g) {
    ...
    for (int i = 0; i < names.size(); i++) {
      ...
      if (printAllEdges) {
        for (int j = 0; j < adjacent.size(); j++) {
          ...
          g2.drawLine((edge.x1 + 5), (edge.y1 + 5), (edge.x2 + 5), (edge.y2 + 5));
        }
      }
    ...
    /*
     * if @path param is not null -> performed Dijkstra's Algorithm, show
     * shortest path
     */
    if (path != null) {
      ...
      for (int k = 0; k < path.size() - 1; k++) {
        ...
        g2.drawLine(v1.getX() + 5, v1.getY() + 5, v2.getX() + 5, v2.getY() + 5);
      }
    }
    ...
    /*
     * if @mst param is not null -> performed Kruskal's Algorithm, show minimum
     * spannign tree
     */
    if (mst != null) {
      ...
      for (int e = 0; e < mst.size(); e++) {
        g2.drawLine(mst.get(e).x1 + 5, mst.get(e).y1 + 5, mst.get(e).x2 + 5, mst.get(e).y2 + 5);
      }
    }
  ...
  /*
   * method for implementing Dijkstra's Algorithm using priority queue, based on
   * pseudocode for the same algorithm by Mark Allen Weiss.
   */
  public void dijkstraAlg(String startCity, String destCity) throws UnderflowException {
   ...
   * spanning tree, based on pseudocode for the same algorithm by Mark Allen
   * Weiss.
   */
